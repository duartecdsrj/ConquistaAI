...
final readonly class AuthSession
{
    ...
    public static function create(string $userId, string $hash, \DateTimeImmutable $expiresAt, ?string $familyId, ?string $deviceName): self
    {
        $id = self::uuid();
        return new self($id, $userId, $familyId ?? $id, $hash, $expiresAt, null, $deviceName);
    }
