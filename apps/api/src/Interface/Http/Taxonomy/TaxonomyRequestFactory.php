...
use AppApplicationTaxonomyDTORequestUpdateTaxonomySubjectRequestDto;
...
final class TaxonomyRequestFactory {
    ...
    public function update(ServerRequestInterface $request, string $id): UpdateTaxonomySubjectRequestDto { $payload=$this->payload($request); if(!is_string($payload['name']??null)||(($payload['parent_id']??null)!==null&&!is_string($payload['parent_id']))||(($payload['description']??null)!==null&&!is_string($payload['description']))) throw new InvalidArgumentException('Campos de assunto invalidos.'); return new UpdateTaxonomySubjectRequestDto($id,$payload['name'],$payload['parent_id']??null,$payload['description']??null); }
