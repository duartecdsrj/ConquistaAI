...
final class DoctrinePerformanceStatisticsRepository implements PerformanceStatisticsRepositoryInterface
{
    ...
    {
        $rows=$this->entityManager->createQueryBuilder()->select('answer.id AS answerId, answer.optionId AS optionId, answer.elapsedSeconds AS elapsedSeconds, attempt.completedAt AS completedAt, question.correctOptionId AS correctOptionId, assignment.taxonomySubjectId AS taxonomySubjectId, notebook.filters AS filters')->from(AnswerRecord::class,'answer')->innerJoin(AttemptRecord::class,'attempt','WITH','attempt.id=answer.attemptId')->innerJoin(NotebookRecord::class,'notebook','WITH','notebook.id=attempt.notebookId')->innerJoin(QuestionRecord::class,'question','WITH','question.id=attempt.questionId')->innerJoin(QuestionTaxonomySubjectRecord::class,'assignment','WITH','assignment.questionId=question.id')->where('attempt.userId=:userId')->andWhere('attempt.completedAt IS NOT NULL')->andWhere('attempt.finalAnswerId=answer.id')->setParameter('userId',$userId)->getQuery()->getArrayResult();
        $answers=[];foreach($rows as $row){$filters=is_array($row['filters']??null)?$row['filters']:[];if(($filters['exam_id']??null)!==$examId)continue;$id=(string)$row['answerId'];if(!isset($answers[$id]))$answers[$id]=['correct'=>isset($row['correctOptionId'])&&$row['optionId']===$row['correctOptionId'],'elapsed'=>(int)$row['elapsedSeconds'],'completedAt'=>$row['completedAt'],'subjects'=>[]];$answers[$id]['subjects'][]=(string)$row['taxonomySubjectId'];}return array_map(static fn(array $a)=>new SyllabusCompletedAnswer($a['correct'],$a['elapsed'],$a['completedAt'],array_values(array_unique($a['subjects']))),array_values($answers));
    }
