...
final class DoctrinePerformanceStatisticsRepository implements PerformanceStatisticsRepositoryInterface
{
    ...
    {
        $rows=$this->entityManager->createQueryBuilder()->select('answer.id AS answerId, answer.optionId AS optionId, answer.elapsedSeconds AS elapsedSeconds, attempt.completedAt AS completedAt, question.correctOptionId AS correctOptionId, assignment.taxonomySubjectId AS taxonomySubjectId')->from(AnswerRecord::class,'answer')->innerJoin(AttemptRecord::class,'attempt','WITH','attempt.id=answer.attemptId')->innerJoin(QuestionRecord::class,'question','WITH','question.id=attempt.questionId')->innerJoin(QuestionTaxonomySubjectRecord::class,'assignment','WITH','assignment.questionId=question.id')->innerJoin(PositionTaxonomyAssignmentRecord::class,'matrix','WITH','matrix.taxonomySubjectId=assignment.taxonomySubjectId')->innerJoin(PositionRecord::class,'position','WITH','position.id=matrix.positionId')->where('attempt.userId=:userId')->andWhere('attempt.completedAt IS NOT NULL')->andWhere('attempt.finalAnswerId=answer.id')->andWhere('position.examId=:examId')->setParameter('userId',$userId)->setParameter('examId',$examId)->getQuery()->getArrayResult();
