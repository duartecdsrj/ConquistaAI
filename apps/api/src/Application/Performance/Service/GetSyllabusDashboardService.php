...
final class GetSyllabusDashboardService {
 ...
 public function getForUser(string $userId, GetSyllabusDashboardRequestDto $request): SyllabusDashboardResponseDto {
  $syllabi=$this->statistics->syllabiWithCompletedAnswersForUser($userId);$selectedId=$request->examId===null?($request->syllabusId??$syllabi[0]->id??null):null;$options=array_map(static fn($item):array=>['id'=>$item->id,'name'=>$item->name,'positionName'=>$item->positionName,'examName'=>$item->examName],$syllabi);
  if($request->examId===null&&($selectedId===null||!in_array($selectedId,array_column($options,'id'),true)))return new SyllabusDashboardResponseDto($options,null,0,0,0,0.0,0.0,false,0,[]);
  $answers=$request->examId!==null?$this->statistics->completedAnswersForUserAndExam($userId,$request->examId):$this->statistics->completedAnswersForUserAndSyllabus($userId,$selectedId);$nodes=$this->statistics->taxonomyHierarchyForSyllabus($selectedId??'');$byId=[];foreach($nodes as $node)$byId[$node->id]=$node;$metrics=[];
