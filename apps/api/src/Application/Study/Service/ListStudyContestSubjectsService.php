...
final class ListStudyContestSubjectsService { public function __construct(private readonly StudyContestSubjectRepositoryInterface $subjects){} /** @return list<StudyContestSubjectResponseDto> */ public function list(string $positionId):array{return array_map(static fn($s)=>new StudyContestSubjectResponseDto($s->id,$s->parentId,$s->name,$s->level,$s->selectionWeight),$this->subjects->listForPosition($positionId));} }
