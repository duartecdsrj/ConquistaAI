...
final class AppendAnswerRepository implements AttemptRepositoryInterface { public ?Answer $answer=null; public function saveAttempt(Attempt $attempt): void {} public function nextNumber(string $userId, string $notebookId, string $questionId): int { return 1; } public function appendAnswer(Answer $answer): void {$this->answer=$answer;} public function listAnswers(string $attemptId): array{return $this->answer===null?[]:[$this->answer];} }
