...
final class SubjectTaxonomyServiceTest extends TestCase
{
    ...
    {
        $hierarchy = new class implements TaxonomyHierarchyRepositoryInterface { public function ancestorIds(string $subjectId): array { return $subjectId === 'dns' ? ['protocols', 'networks', 'informatics'] : []; } };
        $this->expectException(\DomainException::class);
        (new SubjectTaxonomyService())->assertMoveDoesNotCreateCycle('networks', 'dns', $hierarchy);
    }
    public function testScoresEquivalentAccentedNamesAsExactMatch(): void
    {
        self::assertSame(1.0, (new SubjectTaxonomyService())->similarity('Segurança', 'Seguranca'));
    }
    public function testScoresRelatedNamesBelowExactMatch(): void
    {
        self::assertGreaterThan(0.5, (new SubjectTaxonomyService())->similarity('Redes de Computadores', 'Rede de Computador'));
    }
}
