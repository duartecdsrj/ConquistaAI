...
import type { CreateNotebookCommand, Notebook, NotebookStatistics, NotebookStatus, StudyGoal, StudyPlan, StudyRepository } from '../../Domain/Study/StudyRepository'
...
export class AxiosStudyRepository implements StudyRepository {
  ...
  public async create(command: CreateNotebookCommand): Promise<Notebook> { return mapNotebook(await postData<NotebookApi, Record<string, unknown>>('/notebooks', { name: command.name, mode: command.mode, quantity: command.quantity, filters: { ...(command.filters.subjectId ? { subject_id: command.filters.subjectId } : {}), ...(command.filters.board ? { board: command.filters.board } : {}), ...(command.filters.year ? { year: command.filters.year } : {}), ...(command.filters.difficulty ? { difficulty: command.filters.difficulty } : {}) } })) }
