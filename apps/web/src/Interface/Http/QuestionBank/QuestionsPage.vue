<template>
  <q-page class="page">
    ...
    <q-list v-if="questions.length" bordered separator class="questions"><q-expansion-item v-for="(question, index) in questions" :key="question.id" group="questions" header-class="question-header"><template #header><q-item-section><span class="number">QUESTÃO {{ index + 1 }}</span><q-item-label class="statement">{{ question.statement }}</q-item-label><q-item-label caption>{{ question.board || 'Banca não informada' }} · {{ question.year || 'Ano não informado' }} · {{ difficulty(question.difficulty) }}</q-item-label></q-item-section></template><q-card flat class="options"><q-card-section><div v-if="question.assetUrls?.length" class="assets"><QuestionAssetImage v-for="url in question.assetUrls" :key="url" :url="url" /></div><QuestionContent class="question-content" :value="question.statement"/><div v-for="option in question.options" :key="option.id" class="option"><strong>{{ option.label }}.</strong><QuestionContent :value="option.content"/><div v-if="option.assetUrls?.length" class="option-assets"><QuestionAssetImage v-for="url in option.assetUrls" :key="url" :url="url" /></div></div></q-card-section></q-card></q-expansion-item></q-list>
    <div v-if="total > perPage" class="pagination"><q-pagination :model-value="page" :max="Math.ceil(total / perPage)" :max-pages="7" color="primary" boundary-numbers @update:model-value="goTo" /></div>
    <q-card v-if="!loading && !questions.length" flat class="empty"><q-card-section><q-img src="/images/concursos-study-mark.png" width="84px" height="84px" fit="contain" /><div><h2>Nenhuma questão para estes filtros</h2><p>Tente remover filtros ou peça a inclusão de questões publicadas.</p></div></q-card-section></q-card>
  </q-page>
</template>
...
const { error, goTo, load, loading, page, perPage, questions, total } = useQuestions()
...
<style scoped>
.page{max-width:1050px;margin:auto;padding:42px 34px}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:23px}.eyebrow{margin:0 0 7px;color:#7187ad;font-size:11px;font-weight:800;letter-spacing:.1em}.top h1{margin:0;color:#142950;font-size:32px}.top p:not(.eyebrow){margin:8px 0 0;color:#71819e}.filters,.questions,.empty{border:1px solid #e5ecf6;border-radius:17px;background:#fff;box-shadow:0 8px 26px rgba(33,58,105,.04)}.filter-grid{display:grid;grid-template-columns:1.2fr .7fr 1fr auto;gap:12px}.filters .q-btn{border-radius:9px}.results{color:#72829d;font-size:13px;margin:20px 0 10px}.questions{overflow:hidden}.number{color:#2b6ed2;font-size:10px;font-weight:800;letter-spacing:.1em}.statement{margin:8px 0;color:#1b315a;font-size:16px;line-height:1.5}.question-content{margin:12px 0;color:#1b315a}.assets{display:grid;gap:12px;margin:12px 0}.assets :deep(img),.option-assets :deep(img){max-width:100%;max-height:430px;object-fit:contain;border:1px solid #dce6f4;border-radius:10px}.option{display:flex;align-items:flex-start;gap:9px;padding:10px 13px;margin:8px 0;border-radius:10px;background:#f7f9fd;color:#405174}.option :deep(.content){flex:1}.option-assets{width:100%}.empty :deep(.q-card__section){display:flex;align-items:center;gap:22px;padding:32px}.empty h2{margin:0;color:#1a315a;font-size:19px}.empty p{color:#71819e}.pagination{display:flex;justify-content:center;margin:22px 0}.error{margin-top:15px;background:#fff3f2;color:#ae2f25}@media(max-width:660px){.page{padding:28px 16px}.top{align-items:flex-start;flex-direction:column}.filter-grid{grid-template-columns:1fr}.filters .q-btn{height:42px}.empty :deep(.q-card__section){align-items:flex-start;flex-direction:column}}

/* Banco: leitura de questões em página fluida, com filtros em uma faixa discreta. */
.page{max-width:1240px!important}.top{padding:9px 0 23px;border-bottom:1px solid #dce7f6}.filters{border-radius:18px!important;background:rgba(255,255,255,.72)!important}.question-card{border-radius:22px!important;background:rgba(255,255,255,.84)!important}.question-card:hover{transform:translateY(-2px);box-shadow:0 15px 34px rgba(37,71,126,.10)!important}.question-meta{color:#65799d!important}.empty{border-radius:24px!important;background:linear-gradient(145deg,#fff,#eef6ff)!important}
</style>
