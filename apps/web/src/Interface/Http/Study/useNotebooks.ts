...
export function useNotebooks() {
  const startedAt = ref<number | null>(null); const loading = ref(false); const saving = ref(false); const loadingQuestions = ref(false); const submittingAnswer = ref(false); const error = ref(''); const answerMessage = ref(''); const notebooks = ref<readonly Notebook[]>([]); const total = ref(0); const questions = ref<readonly PublishedQuestion[]>([])
  ...
  async function loadQuestions(notebookId: string): Promise<void> {
    loadingQuestions.value = true; error.value = ''; answerMessage.value = ''; questions.value = []; startedAt.value = Date.now()
  ...
  async function submitAnswer(notebookId: string, questionId: string, optionId: string): Promise<boolean> {
    const elapsedSeconds = startedAt.value === null ? 0 : Math.floor((Date.now() - startedAt.value) / 1000)
    submittingAnswer.value = true; error.value = ''; answerMessage.value = ''
    try { await performanceUseCases.submitAnswer.execute(notebookId, questionId, optionId, elapsedSeconds); answerMessage.value = 'Resposta registrada e tentativa concluída.'; return true }
