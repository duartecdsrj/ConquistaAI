...
import type { CreateNotebookCommand, DirectedStudyPlan, Notebook, StudyContestSubject, StudyGoal, StudyPlan } from '../../../Domain/Study/StudyRepository'
...
export function useNotebooks() {
  ...
  const contests = ref<readonly Exam[]>([]); const directedPlans = ref<readonly DirectedStudyPlan[]>([]); const contestSubjects = ref<readonly StudyContestSubject[]>([]); const positions = ref<readonly Position[]>([])
  ...
  async function loadDirectedPlans(): Promise<void> { try { directedPlans.value = await studyUseCases.directedPlans.execute() } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Não foi possível carregar os planos direcionados.' } }
  async function createDirectedPlan(command: { name:string; examId:string; positionId:string }): Promise<DirectedStudyPlan | null> { try { const plan = await studyUseCases.createDirectedPlan.execute(command); directedPlans.value = [plan, ...directedPlans.value.filter(item => item.id !== plan.id)]; return plan } catch (reason) { error.value = reason instanceof Error ? reason.message : 'Não foi possível criar o plano direcionado.'; return null } }
