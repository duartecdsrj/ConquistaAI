  ...
  <q-page class="page">
    ...
    <q-card flat class="catalog-card"><q-card-section>
      ...
      <template v-if="selectedSyllabus">
        ...
        <div v-if="subjects.length" class="chips"><q-chip v-for="subject in subjects" :key="subject.id" color="blue-1" text-color="primary">{{ subject.name }}<q-tooltip v-if="subject.sourceExcerpt">Página {{ subject.sourcePage || 'não informada' }} · {{ subject.sourceExcerpt }}</q-tooltip></q-chip></div><p v-else class="empty">Nenhum assunto para este edital.</p>
      </template>
...
async function selectSyllabus(id: string): Promise<void> { selectedSyllabus.value = id; documentFile.value = null; await chooseSyllabus(id) }
